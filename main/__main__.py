...
async def start(bot, message):
  await message.reply_photo("https://telegra.ph/file/0a9f90dd90d5281e16747.jpg",caption=START,reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Help", callback_data="help_menu")]]))

@app.on_message(filters.command("help"))
async def help(bot, message):
  await message.reply_photo("https://telegra.ph/file/0a9f90dd90d5281e16747.jpg",caption=HELP,reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Back", callback_data="start_menu")]]))

@app.on_message(filters.command("logo") & filters.incoming & filters.text & ~filters.forwarded & (
  filters.group | filters.private))
async def logo(bot, message):
  try:
    text = message.text.replace("logo","").replace("/","").replace("@Emo_Logo_Maker_Bot","").strip().upper()
    ...
    await message.reply_photo(logo,caption="**🖼 Logo Generated By @Emo_Logo_Maker_Bot**",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Upload As File 📁", callback_data=f"flogo {logo_id}")]]))
    await x.delete()
  except FloodWait:
    pass
...
async def logo(bot, message):
  try:
    text = message.text.replace("logosq","").replace("/","").replace("@Emo_Logo_Maker_Bot","").strip().upper()
...
async def logo_doc(_,query):
  ...
  try:
    ...
    await query.message.reply_document(link,caption="**🖼 Logo Generated By @Emo_Logo_Maker_Bot**")
  except FloodWait:
    pass
  except Exception as e:
    try:
